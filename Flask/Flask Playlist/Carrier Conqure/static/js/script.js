...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (uploadBtn) {
    uploadBtn.addEventListener("click", async () => {
      ...
      if (!file || !prompt) {
        alert("Please enter a prompt and upload a file.");
        return;
      }
      ...
      if (result.result) {
        chatBox.innerHTML += `<div class="message bot-message">${result.result}</div>`;
      } else {
        chatBox.innerHTML += `<div class="message bot-message">Error: ${result.error}</div>`;
      }
    });
  }
});
